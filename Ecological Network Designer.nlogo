...
to go
  ...
  foreach sort turtles [ ask ? [
    set color hsb (360 * i / count turtles) 100 100
    set i i + 1
  ]]
...
to add-relation
  ...
  if mouse-is-down? and is-turtle? new-link-from [
    ...
    watch target
    if is-link? new-link and target != ([end2] of new-link) [
      show "hi"
      ask new-link [ die ]
    ]
    if not is-link? new-link and target != new-link-from [
      ask new-link-from [
        create-link-to target [ set new-link self ]
      ]
    ]
  ]
  if mouse-released? [
...
to remove-species
  ...
  ask turtle-set target [
    ifelse distancexy mouse-xcor mouse-ycor < 1 [
      if mouse-is-down? [
        watch-me
      ]
      if mouse-released? [
...
to-report closest-turtle [ x y ]
  report min-one-of turtles [ distancexy mouse-xcor mouse-ycor ]
end

to-report serialize-food-web
  report [(list color [ color ] of in-link-neighbors)] of turtles
end

to create-rps-cycle
  ca
  let n ceiling (num-turtles / 2) - 1
  cro num-turtles [
    fd 10
    foreach n-values n [1 + ?] [
      create-link-to turtle ((who + ?) mod num-turtles)
    ]
  ]
end
...
mode
mode
"add-species" "add-relation" "remove-species" "remove-relation"
1
